...
def upload_shanyrak_photos(
    ...
    update_result = svc.repository.update_shanyrak(
        shanyrak_id, jwt_data.user_id, data={"media": media_urls}
    )
...
def delete_shanyrak_photos(
    ...
    shanyrak = svc.repository.get_shanyrak(
    ...
    update_result = svc.repository.update_shanyrak(
        shanyrak_id, jwt_data.user_id, data={"media": media}
    )
    if not update_result.acknowledged:
        raise HTTPException(
            status_code=404,
            detail=f"Error occured while deleting photos from shanyrak {shanyrak}",
        )
    return Response(status_code=200)
